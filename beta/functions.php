...
define("DB_USERNAME","cypress");
define("DB_PASSWORD","[dennected24]");
define("DB_DATABASE","consequence");
...
function login() {
	if ( array_key_exists("name",$_GET) && $_GET["name"] != "" && array_key_exists("password", $_GET) && $_GET["password"] != ""){
		...
		if($typed_password == $real_password) {
			setcookie("cookie",$cookie,time()+COOKIE_TIME);
			mysql_query($query);
...
function newUser() {
	if ( array_key_exists("name",$_GET) && $_GET["name"] != "" && array_key_exists("password", $_GET) && $_GET["password"] != ""){
		...
		if( getNameId( $name ) && !userIsDeleted( getNameCookie($name) ) ) {
			echo "<p id='alert'>That username is taken. Chose another one or <a href='?page=login'>login</a>.</p>";
			return false;
		}
		else {
